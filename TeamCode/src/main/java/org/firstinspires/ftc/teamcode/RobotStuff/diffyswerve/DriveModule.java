...
public class DriveModule {
   ...
   ; //ticks per WHEEL revolution( write function for every moment active motors directions they are going and the ratio of the directions)
    ...
    public final double MM_WHEEL_DIAMETER = 3 * 25.4;
    public final double MM_PER_WHEEL_REV = MM_WHEEL_DIAMETER * Math.PI;
    public final double MM_PER_TICK = MM_PER_WHEEL_REV/TICKS_PER_WHEEL_REV;
    ...
    public DriveModule(Robot robot, ModuleSide moduleSide, boolean debuggingMode) {
       ...
       // if (debuggingMode) {
        //    dataLogger = new DataLogger(moduleSide + "ModuleLog");
         //   System.out.printf("Trans Vector FC X", dataLogger);
    ...
    public void setMotorPowers (Vector2d powerVector) {
        ...
        Vector2d motor1Unscaled = powerVector.projection(LMOTOR_1_VECTOR);
        Vector2d motor2Unscaled = powerVector.projection(LMOTOR_2_VECTOR);
        ...
        if (motorPowersScaled[0].getAngleDouble(Angle.AngleType.NEG_180_TO_180_CARTESIAN) != LMOTOR_1_VECTOR.getAngleDouble(Angle.AngleType.NEG_180_TO_180_CARTESIAN)) {
            motor1power *= -1;
        }
        if (motorPowersScaled[1].getAngleDouble(Angle.AngleType.NEG_180_TO_180_CARTESIAN) != LMOTOR_2_VECTOR.getAngleDouble(Angle.AngleType.NEG_180_TO_180_CARTESIAN)) {
            motor2power *= -1;
        }
